...
@section('title', 'تعديل سجل امتثال وسلامة')

@php
    $breadcrumbTitle = 'تعديل سجل امتثال وسلامة';
@endphp
            ...
            <div class="col-12">
                <div class="card log-card">
                        ...
                        <div class="d-flex flex-wrap align-items-center justify-content-between gap-2 mb-0">
                            <div>
                                <div class="log-title">
                                    ...
                                    تعديل سجل امتثال وسلامة
                                </div>
                                <div class="log-subtitle">
                                    قم بتعديل بيانات الامتثال والسلامة
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="card-body p-4">
                        <form action="{{ route('admin.compliance-safeties.update', $complianceSafety) }}" method="POST" id="complianceSafetyForm">
                            @csrf
                            @method('PUT')
                                ...
                                <div class="row g-3">
                                    <div class="col-md-6">
                                        ...
                                        <select name="operator_id" class="form-select @error('operator_id') is-invalid @enderror"
                                            ...
                                            @foreach($operators as $operator)
                                                <option value="{{ $operator->id }}" 
                                                        {{ old('operator_id', $complianceSafety->operator_id) == $operator->id ? 'selected' : '' }}>
                                                    {{ $operator->name }}
                                        ...
                                        @if(auth()->user()->isCompanyOwner())
                                            <input type="hidden" name="operator_id" value="{{ $complianceSafety->operator_id }}">
                                        @endif
                                        @error('operator_id')
                                ...
                                <div class="row g-3">
                                    <div class="col-md-6">
                                        ...
                                        <input type="date" name="last_inspection_date" 
                                               ...
                                               value="{{ old('last_inspection_date', $complianceSafety->last_inspection_date ? $complianceSafety->last_inspection_date->format('Y-m-d') : '') }}">
                                    ...
                                    <div class="col-md-6">
                                        ...
                                        <input type="text" name="inspection_authority" 
                                               ...
                                               value="{{ old('inspection_authority', $complianceSafety->inspection_authority) }}" 
                                    ...
                                    <div class="col-md-12">
                                        ...
                                        <textarea name="inspection_result" 
                                                  ...
                                                  rows="4"
                                                  placeholder="تفاصيل نتيجة الزيارة التفقدية">{{ old('inspection_result', $complianceSafety->inspection_result) }}</textarea>
                                    ...
                                    <div class="col-md-12">
                                        ...
                                        <textarea name="violations" 
                                                  ...
                                                  rows="4"
                                                  placeholder="تفاصيل المخالفات المسجلة إن وجدت">{{ old('violations', $complianceSafety->violations) }}</textarea>
                                        @error('violations')
                                            <div class="invalid-feedback d-block">{{ $message }}</div>
                                        @enderror
                                    </div>
                    ...
                    success: function(response) {
                        if (response.success) {
                            if (typeof window.showToast === 'function') {
                                window.showToast(response.message || 'تم تحديث سجل الامتثال والسلامة بنجاح', 'success');
                            } else {
                                alert(response.message || 'تم تحديث سجل الامتثال والسلامة بنجاح');
                            }
                            setTimeout(function() {
