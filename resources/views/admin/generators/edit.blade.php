                        ...
                        <div class="d-flex flex-wrap align-items-center justify-content-between gap-2">
                            <div>
                                ...
                                <div class="gen-subtitle">
                                    قم بتعديل بيانات المولد الكهربائي
                                </div>
                            </div>
                    ...
                    <div class="tab-content" id="generatorTabsContent">
                        ...
                        <div class="row g-3">
                            ...
                            <div class="col-md-6">
                                ...
                                <input type="text" name="generator_number" id="generator_number" class="form-control @error('generator_number') is-invalid @enderror" 
                                       value="{{ old('generator_number', $generator->generator_number) }}" readonly placeholder="يتم توليده تلقائياً">
                                ...
                                <div class="col-md-6">
                                    ...
                                    <select name="operator_id" class="form-select @error('operator_id') is-invalid @enderror">
                                        ...
                                        @foreach($operators as $operator)
                                            <option value="{{ $operator->id }}" {{ old('operator_id', $generator->operator_id) == $operator->id ? 'selected' : '' }}>
                                                {{ $operator->name }}
                                            </option>
                                        @endforeach
                                    </select>
                            ...
                            <div class="col-md-6">
                                ...
                                <select name="status" class="form-select @error('status') is-invalid @enderror">
                                    ...
                                    @foreach($constants['status'] ?? [] as $status)
                                        <option value="{{ $status->id }}" {{ old('status', $generator->status) == $status->id ? 'selected' : '' }}>
                                            {{ $status->label }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div class="col-md-12">
                                ...
                                <textarea name="description" class="form-control @error('description') is-invalid @enderror" rows="3">{{ old('description', $generator->description) }}</textarea>
                        ...
                        <div class="row g-3">
                            <div class="col-md-6">
                                ...
                                <input type="number" step="0.01" name="capacity_kva" class="form-control @error('capacity_kva') is-invalid @enderror" 
                                       value="{{ old('capacity_kva', $generator->capacity_kva) }}" min="0">
                            ...
                            <div class="col-md-6">
                                ...
                                <input type="number" step="0.01" name="power_factor" class="form-control @error('power_factor') is-invalid @enderror" 
                                       value="{{ old('power_factor', $generator->power_factor) }}" min="0" max="1">
                            ...
                            <div class="col-md-6">
                                ...
                                <input type="number" name="voltage" class="form-control @error('voltage') is-invalid @enderror" 
                                       value="{{ old('voltage', $generator->voltage) }}" min="0">
                            ...
                            <div class="col-md-6">
                                ...
                                <input type="number" name="frequency" class="form-control @error('frequency') is-invalid @enderror" 
                                       value="{{ old('frequency', $generator->frequency) }}" min="0">
                            ...
                            <div class="col-md-6">
                                ...
                                <select name="engine_type" class="form-select @error('engine_type') is-invalid @enderror">
                                    ...
                                    @foreach($constants['engine_type'] ?? [] as $engineType)
                                        <option value="{{ $engineType->id }}" {{ old('engine_type', $generator->engine_type) == $engineType->id ? 'selected' : '' }}>
                                            {{ $engineType->label }}
                                        </option>
                                    @endforeach
                                    {{-- Fallback للقيم القديمة --}}
                                    @if(($constants['engine_type'] ?? collect())->isEmpty())
                                        <option value="Perkins" {{ old('engine_type', $generator->engine_type) === 'Perkins' ? 'selected' : '' }}>Perkins</option>
                                        <option value="Volvo" {{ old('engine_type', $generator->engine_type) === 'Volvo' ? 'selected' : '' }}>Volvo</option>
                                        <option value="Caterpillar" {{ old('engine_type', $generator->engine_type) === 'Caterpillar' ? 'selected' : '' }}>Caterpillar</option>
                                        <option value="DAF" {{ old('engine_type', $generator->engine_type) === 'DAF' ? 'selected' : '' }}>DAF</option>
                                        <option value="MAN" {{ old('engine_type', $generator->engine_type) === 'MAN' ? 'selected' : '' }}>MAN</option>
                                        <option value="SCAINA" {{ old('engine_type', $generator->engine_type) === 'SCAINA' ? 'selected' : '' }}>SCAINA</option>
                                    @endif
                                </select>
                        ...
                        <div class="row g-3">
                            <div class="col-md-6">
                                ...
                                <input type="text" name="manufacturing_year" id="manufacturing_year" class="form-control @error('manufacturing_year') is-invalid @enderror" 
                                       value="{{ old('manufacturing_year', $generator->manufacturing_year) }}" placeholder="اختر السنة">
                            ...
                            <div class="col-md-6">
                                ...
                                <select name="injection_system" class="form-select @error('injection_system') is-invalid @enderror">
                                    ...
                                    @foreach($constants['injection_system'] ?? [] as $injection)
                                        <option value="{{ $injection->id }}" {{ old('injection_system', $generator->injection_system) == $injection->id ? 'selected' : '' }}>
                                            {{ $injection->label }}
                                        </option>
                                    @endforeach
                                    {{-- Fallback للقيم القديمة --}}
                                    @if(($constants['injection_system'] ?? collect())->isEmpty())
                                        <option value="عادي" {{ old('injection_system', $generator->injection_system) === 'عادي' ? 'selected' : '' }}>عادي</option>
                                        <option value="كهربائي" {{ old('injection_system', $generator->injection_system) === 'كهربائي' ? 'selected' : '' }}>كهربائي</option>
                                        <option value="هجين" {{ old('injection_system', $generator->injection_system) === 'هجين' ? 'selected' : '' }}>هجين</option>
                                    @endif
                                </select>
                            ...
                            <div class="col-md-6">
                                ...
                                <input type="number" step="0.01" name="fuel_consumption_rate" class="form-control @error('fuel_consumption_rate') is-invalid @enderror" 
                                       value="{{ old('fuel_consumption_rate', $generator->fuel_consumption_rate) }}" min="0">
                            ...
                            <div class="col-md-6">
                                ...
                                <input type="number" step="0.001" name="ideal_fuel_efficiency" class="form-control @error('ideal_fuel_efficiency') is-invalid @enderror" 
                                       value="{{ old('ideal_fuel_efficiency', $generator->ideal_fuel_efficiency ?? '0.5') }}" min="0" max="10" placeholder="0.5">
                            ...
                            <div class="col-md-6">
                                ...
                                <input type="number" name="internal_tank_capacity" class="form-control @error('internal_tank_capacity') is-invalid @enderror" 
                                       value="{{ old('internal_tank_capacity', $generator->internal_tank_capacity) }}" min="0">
                            ...
                            <div class="col-md-6">
                                ...
                                <select name="measurement_indicator" class="form-select @error('measurement_indicator') is-invalid @enderror">
                                    ...
                                    @foreach($constants['measurement_indicator'] ?? [] as $indicator)
                                        <option value="{{ $indicator->id }}" {{ old('measurement_indicator', $generator->measurement_indicator) == $indicator->id ? 'selected' : '' }}>
                                            {{ $indicator->label }}
                                        </option>
                                    @endforeach
                                    {{-- Fallback للقيم القديمة --}}
                                    @if(($constants['measurement_indicator'] ?? collect())->isEmpty())
                                        <option value="غير متوفر" {{ old('measurement_indicator', $generator->measurement_indicator) === 'غير متوفر' ? 'selected' : '' }}>غير متوفر</option>
                                        <option value="متوفر ويعمل" {{ old('measurement_indicator', $generator->measurement_indicator) === 'متوفر ويعمل' ? 'selected' : '' }}>متوفر ويعمل</option>
                                        <option value="متوفر ولا يعمل" {{ old('measurement_indicator', $generator->measurement_indicator) === 'متوفر ولا يعمل' ? 'selected' : '' }}>متوفر ولا يعمل</option>
                                    @endif
                                </select>
                        ...
                        <div class="row g-3">
                            <div class="col-md-6">
                                ...
                                <select name="technical_condition" class="form-select @error('technical_condition') is-invalid @enderror">
                                    ...
                                    @foreach($constants['technical_condition'] ?? [] as $condition)
                                        <option value="{{ $condition->id }}" {{ old('technical_condition', $generator->technical_condition) == $condition->id ? 'selected' : '' }}>
                                            {{ $condition->label }}
                                        </option>
                                    @endforeach
                                    {{-- Fallback للقيم القديمة --}}
                                    @if(($constants['technical_condition'] ?? collect())->isEmpty())
                                        <option value="ممتازة" {{ old('technical_condition', $generator->technical_condition) === 'ممتازة' ? 'selected' : '' }}>ممتازة</option>
                                        <option value="جيدة جدا" {{ old('technical_condition', $generator->technical_condition) === 'جيدة جدا' ? 'selected' : '' }}>جيدة جدا</option>
                                        <option value="جيدة" {{ old('technical_condition', $generator->technical_condition) === 'جيدة' ? 'selected' : '' }}>جيدة</option>
                                        <option value="متوسطة" {{ old('technical_condition', $generator->technical_condition) === 'متوسطة' ? 'selected' : '' }}>متوسطة</option>
                                        <option value="سيئة" {{ old('technical_condition', $generator->technical_condition) === 'سيئة' ? 'selected' : '' }}>سيئة</option>
                                    @endif
                                </select>
                            ...
                            <div class="col-md-6">
                                ...
                                <div class="image-preview-container mt-2" id="engine_data_plate_preview" style="{{ $generator->engine_data_plate_image ? 'display: block;' : 'display: none;' }}">
                                    <img src="{{ $generator->engine_data_plate_image ? Storage::url($generator->engine_data_plate_image) : '' }}" alt="معاينة" class="image-preview">
                            ...
                            <div class="col-md-6">
                                ...
                                <div class="image-preview-container mt-2" id="generator_data_plate_preview" style="{{ $generator->generator_data_plate_image ? 'display: block;' : 'display: none;' }}">
                                    <img src="{{ $generator->generator_data_plate_image ? Storage::url($generator->generator_data_plate_image) : '' }}" alt="معاينة" class="image-preview">
                        ...
                        <div class="row g-3">
                            <div class="col-md-6">
                                ...
                                <select name="control_panel_available" class="form-select @error('control_panel_available') is-invalid @enderror">
                                    <option value="0" {{ old('control_panel_available', $generator->control_panel_available ? '1' : '0') == '0' ? 'selected' : '' }}>غير متوفرة</option>
                                    <option value="1" {{ old('control_panel_available', $generator->control_panel_available ? '1' : '0') == '1' ? 'selected' : '' }}>متوفرة</option>
                                </select>
                            ...
                            <div class="col-md-6">
                                ...
                                <select name="control_panel_type" class="form-select @error('control_panel_type') is-invalid @enderror">
                                    ...
                                    @foreach($constants['control_panel_type'] ?? [] as $panelType)
                                        <option value="{{ $panelType->id }}" {{ old('control_panel_type', $generator->control_panel_type) == $panelType->id ? 'selected' : '' }}>
                                            {{ $panelType->label }}
                                        </option>
                                    @endforeach
                                    {{-- Fallback للقيم القديمة --}}
                                    @if(($constants['control_panel_type'] ?? collect())->isEmpty())
                                        <option value="Deep Sea" {{ old('control_panel_type', $generator->control_panel_type) === 'Deep Sea' ? 'selected' : '' }}>Deep Sea</option>
                                        <option value="ComAp" {{ old('control_panel_type', $generator->control_panel_type) === 'ComAp' ? 'selected' : '' }}>ComAp</option>
                                        <option value="Datakom" {{ old('control_panel_type', $generator->control_panel_type) === 'Datakom' ? 'selected' : '' }}>Datakom</option>
                                        <option value="Analog" {{ old('control_panel_type', $generator->control_panel_type) === 'Analog' ? 'selected' : '' }}>Analog</option>
                                    @endif
                                </select>
                            ...
                            <div class="col-md-6">
                                ...
                                <select name="control_panel_status" class="form-select @error('control_panel_status') is-invalid @enderror">
                                    ...
                                    @foreach($constants['control_panel_status'] ?? [] as $panelStatus)
                                        <option value="{{ $panelStatus->id }}" {{ old('control_panel_status', $generator->control_panel_status) == $panelStatus->id ? 'selected' : '' }}>
                                            {{ $panelStatus->label }}
                                        </option>
                                    @endforeach
                                    {{-- Fallback للقيم القديمة --}}
                                    @if(($constants['control_panel_status'] ?? collect())->isEmpty())
                                        <option value="تعمل" {{ old('control_panel_status', $generator->control_panel_status) === 'تعمل' ? 'selected' : '' }}>تعمل</option>
                                        <option value="لا تعمل" {{ old('control_panel_status', $generator->control_panel_status) === 'لا تعمل' ? 'selected' : '' }}>لا تعمل</option>
                                    @endif
                                </select>
                            ...
                            <div class="col-md-6">
                                ...
                                <div class="image-preview-container mt-2" id="control_panel_preview" style="{{ $generator->control_panel_image ? 'display: block;' : 'display: none;' }}">
                                    <img src="{{ $generator->control_panel_image ? Storage::url($generator->control_panel_image) : '' }}" alt="معاينة" class="image-preview">
                            ...
                            <div class="col-md-6">
                                ...
                                <input type="number" name="operating_hours" class="form-control @error('operating_hours') is-invalid @enderror" 
                                       value="{{ old('operating_hours', $generator->operating_hours) }}" min="0">
                                        ...
                                        <div class="row g-3">
                                            <div class="col-md-6">
                                                ...
                                                <select name="external_fuel_tank" id="external_fuel_tank" class="form-select @error('external_fuel_tank') is-invalid @enderror">
                                                    <option value="0" {{ old('external_fuel_tank', $generator->external_fuel_tank ? '1' : '0') == '0' ? 'selected' : '' }}>لا</option>
                                                    <option value="1" {{ old('external_fuel_tank', $generator->external_fuel_tank ? '1' : '0') == '1' ? 'selected' : '' }}>نعم</option>
                                                </select>
                                                
                                            </div>
                                            <div class="col-md-6" id="fuel_tanks_count_wrapper" style="{{ $generator->external_fuel_tank ? 'display: block;' : 'display: none;' }}">
                                                ...
                                                <select name="fuel_tanks_count" id="fuel_tanks_count" class="form-select @error('fuel_tanks_count') is-invalid @enderror">
                                                    ...
                                                    @for($i = 1; $i <= 10; $i++)
                                                        <option value="{{ $i }}" {{ old('fuel_tanks_count', $generator->fuelTanks->count() > 0 ? $generator->fuelTanks->count() : null) == $i ? 'selected' : '' }}>{{ $i }}</option>
                                                    @endfor
                                                </select>
                                                
                                            </div>
                                            <!-- حقل hidden لإرسال القيمة الافتراضية عندما يكون external_fuel_tank = 0 -->
                                            <input type="hidden" id="fuel_tanks_count_hidden" value="{{ old('fuel_tanks_count', $generator->fuel_tanks_count ?? 0) }}">
                                        </div>
                                    </div>
                                </div>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        function refreshCSRFToken() {
            fetch('{{ route("admin.generators.edit", $generator) }}', {
                method: 'GET',
                headers: {
                ...
                success: function(response) {
                    if (response.success) {
                        if (typeof window.showToast === 'function') {
                            window.showToast(response.message || 'تم تحديث المولد بنجاح', 'success');
                        } else {
                            alert(response.message || 'تم تحديث المولد بنجاح');
                        }
                        setTimeout(function() {
        ...
        @if($generator->external_fuel_tank && $generator->fuelTanks->count() > 0)
            renderFuelTanks({{ $generator->fuelTanks->count() }});
            @foreach($generator->fuelTanks as $index => $tank)
