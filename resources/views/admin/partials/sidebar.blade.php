    ...
    <div class="main-sidebar" id="sidebar-scroll">
        <nav class="main-menu-container nav nav-pills flex-column sub-open">
            ...
            <ul class="main-menu">
                ...
                {{-- ============================================ --}}
                {{-- 3. إدارة العمليات (لجميع المستخدمين المصرح لهم) - لا تظهر للمشغل هنا، تظهر في قسم المشغل --}}
                {{-- ============================================ --}}
                @if(!$u->isCompanyOwner())
                    <li class="slide__category mt-3">
                ...
                {{-- ============================================ --}}
                {{-- 3. المشغل (للمشغل فقط) - إعادة ترتيب منطقي --}}
                {{-- ============================================ --}}
                @if($u->isCompanyOwner())
                    <li class="slide__category mt-3">
                    ...
                    {{-- الموظفون والفنيون (رابط مباشر للمشغل المعتمد) --}}
                    @can('viewAny', App\Models\User::class)
                        <li class="slide {{ $isActive('admin.users.*') }}">
                            <a href="{{ route('admin.users.index') }}" class="side-menu__item">
                                <svg xmlns="http://www.w3.org/2000/svg" class="side-menu__icon" width="24" height="24" viewBox="0 0 24 24"><path d="M16 7a4 4 0 1 1-8 0 4 4 0 0 1 8 0zM12 14a7 7 0 0 0-7 7h14a7 7 0 0 0-7-7z"/></svg>
                                <span class="side-menu__label">الموظفون والفنيون</span>
                            </a>
                        </li>
                    @endcan
                    ...
                    {{-- ============================================ --}}
                    {{-- 4. إدارة العمليات (للمشغل) --}}
                    {{-- ============================================ --}}
                    <li class="slide__category mt-3">
                        <span class="side-menu__label text-muted text-xs opacity-70">إدارة العمليات</span>
                    </li>

                    {{-- إدارة المشغل (قائمة فرعية) - تجميع وحدات التوليد والمولدات والمستخدمين --}}
                    <li class="slide has-sub {{ $isOpen('admin.generation-units.*') || $isOpen('admin.generators.*') || $isOpen('admin.users.*') || $isOpen('admin.operators.tariff-prices.*') ? 'open' : '' }}">
                        <a href="javascript:void(0);" class="side-menu__item {{ $isActive('admin.generation-units.*') || $isActive('admin.generators.*') || $isActive('admin.users.*') || $isActive('admin.operators.tariff-prices.*') ? 'active' : '' }}">
                            <svg xmlns="http://www.w3.org/2000/svg" class="side-menu__angle" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="m15 18-6-6 6-6"/></svg>
                        ...
                        <ul class="slide-menu child1" style="{{ ($show('admin.generation-units.*') || $show('admin.generators.*') || $show('admin.users.*') || $show('admin.operators.tariff-prices.*')) ? 'display:block' : '' }}">
                            @can('viewAny', App\Models\GenerationUnit::class)
                                <li class="slide">
